...
def n_palavras_diferentes(lista_palavras):
    ...
    freq = dict()  #dict cria um dicionário vazio chamado freq para contar a frequência de cada palavra na lista.
    for palavra in lista_palavras:
        p = palavra.lower()
        if p in freq:
            freq[p] += 1
...
def compara_assinatura(as_a, as_b):
    ...
    ambas = 0
    for i in range(6):
        ambas = ambas + abs(as_a[i] - as_b[i]) / (i + 1)
...
def avalia_textos(textos, ass_cp):
    '''IMPLEMENTAR. Essa funcao recebe uma lista de textos e uma assinatura ass_cp e deve devolver o numero (1 a n) do texto com maior probabilidade de ter 
    sido infectado por COH-PIAH.'''
    infec = []
    for texto in textos:
        ass_texto = calcula_assinatura(texto)
        infec.append(compara_assinatura(ass_texto, ass_cp))
